...
class CurriculumManager:
    ...
    def __init__(self, env, on_phase_change_callback=None):
        ...
        self.phases = {
            # Phase 1: Core Approach Learning (YOUR EXACT SPECIFICATION - 25% success threshold)
            "milestone_0_percent": {
                "timesteps": 30_000,
                "success_threshold": 0.25,  # YOUR THRESHOLD: 25% success to advance
                ...
                "spawn_radius": 0.11,  # ACTUAL: NEAR SPAWN 8-14cm (avg 11cm)
                "objects": ["cube_only"],
                ...
                # Gating parameters per phase - INCREASED for robust learning
            ...
            # Phase 2: Environmental Robustness (YOUR EXACT SPECIFICATION - 10-30% success threshold)
            "milestone_10_percent": {
                "timesteps": 30_000,
                "success_threshold": 0.25,  # FIXED: Should be 25% success to advance, not 10%
                "focus": "mass_variation_introduction",
                "description": "Add mass variation - spawn_radius 0.25m, mass random 30-70g",
                "collision_rewards": {"gripper_object_contact": 0.2, "arm_object_collision": -1.0},
                "termination_on_bad_collision": False,
                "spawn_radius": 0.25,  # YOUR SPEC: 0.25m
                "objects": ["cube_only"],
                "mass_range": (30, 70),  # YOUR SPEC: random 30-70g
                "color_randomization": False,  # Fixed
                "lighting_randomization": False,  # Fixed
                "friction_randomization": False,  # Fixed  
            ...
            "milestone_15_percent": {
                "timesteps": 30_000,
                "success_threshold": 0.15,  # YOUR THRESHOLD: 15% success to advance
                "focus": "visual_variation_introduction",
                "description": "Add color randomization - spawn_radius 0.30m, color random RGB",
                "collision_rewards": {"gripper_object_contact": 0.2, "arm_object_collision": -1.0},
                "termination_on_bad_collision": False,
                "spawn_radius": 0.30,  # YOUR SPEC: 0.30m
                "objects": ["cube_only"],
                "mass_range": (30, 70),  # Continue mass variation
                "color_randomization": True,  # YOUR SPEC: Add color randomization
                "lighting_randomization": False,  # Fixed
                "friction_randomization": False,  # Fixed
                "domain_randomization": True,
                "min_phase_episodes": 100,  # Increased from 80
            ...
            "milestone_20_percent": {
                "timesteps": 35_000,
                "success_threshold": 0.20,  # YOUR THRESHOLD: 20% success threshold
                "focus": "physics_variation_introduction", 
                "description": "Add lighting + friction - spawn_radius 0.35m, lighting + friction random",
                "collision_rewards": {"gripper_object_contact": 0.2, "arm_object_collision": -1.0},
                "termination_on_bad_collision": False,
                "spawn_radius": 0.35,  # YOUR SPEC: 0.35m
                "objects": ["cube_only"],
                "mass_range": (20, 100),  # Expanded mass range
                "color_randomization": True,
                "lighting_randomization": True,  # YOUR SPEC: Add lighting variation
                "friction_randomization": True,  # YOUR SPEC: Add friction randomization
                "domain_randomization": True,
            ...
            "milestone_25_percent": {
                ...
                "collision_rewards": {"gripper_object_contact": 0.2, "arm_object_collision": -1.0},
                "termination_on_bad_collision": False,
                "spawn_radius": 0.40,  # YOUR SPEC: 0.40m
                "objects": ["cube", "sphere"],  # YOUR SPEC: Add sphere
                "mass_range": (20, 150),
                "color_randomization": True,
            ...
            # Phase 3: Full Robustness (YOUR EXACT SPECIFICATION - 30%+ success)
            "milestone_30_percent": {
                "timesteps": 40_000,
                "success_threshold": 0.30,  # YOUR THRESHOLD: 30% success to advance to grasping
                "focus": "full_approach_robustness",
                "description": "Full robustness - spawn_radius 0.45m, [cube, sphere, cylinder] + full randomization",
                "collision_rewards": {"gripper_object_contact": 0.2, "arm_object_collision": -1.0},
                "termination_on_bad_collision": False,
                "spawn_radius": 0.45,  # YOUR SPEC: 0.45m
                "objects": ["cube", "sphere", "cylinder"],  # YOUR SPEC: All object types
                "mass_range": (20, 200),
                "color_randomization": True,
                "lighting_randomization": True,
                "friction_randomization": True,
            ...
            "grasping": {
                ...
                "success_threshold": 0.35,  # FIXED: More realistic with gentler spawn progression (was 0.50)
            ...
            "manipulation": {
                ...
                "success_threshold": 0.70,  # FIXED: 70% for real-world transfer
            ...
            "mastery": {
                ...
                "success_threshold": 0.85,  # FIXED: 85% for robust deployment
        ...
        # CRITICAL FIX: Ensure curriculum level is applied immediately
    ...
    def update(self, success_rate: float = None, collision_info: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        # FIXED: Primary advancement with proper approach success metrics
        ...
        # CRITICAL FIX: Improved stability gating with sustained success requirement
        ...
        # ENHANCED: Require ALL gating conditions PLUS sustained success
        ...
        if meets_threshold and has_min_episodes and has_min_time and passed_cooldown and sustained_success:
            ...
            # Fallback: time-based advancement for approach phase only if really stuck
        ...
        elif False:  # Disabled the old complex logic
            # For other phases, use task success rate
            phase_complete = True
        ...
        # For grasping phase, use proper grasp success metrics
            ...
            # FIXED: More lenient grasping advancement since model needs to learn this new behavior
        ...
        if phase_complete:
            ...
            if next_phase:
                ...
                # CRITICAL FIX: Reset ALL tracking for new phase - must be more aggressive
                self.success_history = []  # Clear success history to start fresh
                print(f"   ✅ Success tracking reset for new phase")

                # CRITICAL FIX: Also clear collision history to prevent immediate re-advancement
                ...
                # CRITICAL FIX: Reset sustained success tracking for new phase
                self._sustained_success_count = 0
                print(f"   ✅ Sustained success counter reset for new phase")

                # CRITICAL FIX: Force curriculum level sync after phase change to ensure immediate application
                sync_success = self._force_curriculum_level_sync()
                if not sync_success:
                    print(f"   ⚠️ WARNING: Curriculum level sync failed after phase change!")

                # CRITICAL FIX: Apply new phase settings to eval environment if available
                self._sync_with_eval_environment()
                
                # CRITICAL FIX: Notify training script to reset metrics
    ...
    def _analyze_recent_collisions(self) -> Dict[str, float]:
        """Enhanced collision analysis with proper approach success tracking"""
        ...
        for collision_data in recent_collisions:
            if isinstance(collision_data, dict):
                ...
                approach_threshold = collision_data.get("approach_threshold", 0.10)  # FIXED: Match proximity bonus threshold
    ...
    def _apply_phase_settings(self):
        ...
        # CRITICAL FIX: Match environment curriculum level ranges exactly
        # < 0.15: NEAR SPAWN (8-14cm), 0.15-0.25: INTERMEDIATE SPAWN 1 (15-25cm), 
        # 0.25-0.40: INTERMEDIATE SPAWN 2 (±8cm), > 0.40: FULL AREA SPAWN
        phase_levels = {
            "milestone_0_percent": 0.05,   # NEAR SPAWN: 8-14cm radius (< 0.15) - YOUR SPEC: 0.15m spawn_radius
            "milestone_5_percent": 0.16,   # INTERMEDIATE SPAWN 1: 15-25cm radius - FIXED to match 0.20m spawn_radius spec  
            "milestone_10_percent": 0.18,  # INTERMEDIATE SPAWN 1: 15-25cm radius (0.15-0.25) - YOUR SPEC: 0.25m
            "milestone_15_percent": 0.20,  # INTERMEDIATE SPAWN 1: 15-25cm radius (0.15-0.25) - YOUR SPEC: 0.30m
            "milestone_20_percent": 0.22,  # INTERMEDIATE SPAWN 1: 15-25cm radius (0.15-0.25) - YOUR SPEC: 0.35m
            "milestone_25_percent": 0.30,  # INTERMEDIATE SPAWN 2: ±8cm square area (0.25-0.40) - YOUR SPEC: 0.40m
            "milestone_30_percent": 0.35,  # INTERMEDIATE SPAWN 2: ±8cm square area (0.25-0.40) - YOUR SPEC: 0.45m
        ...
        # CRITICAL FIX: Use robust curriculum level setting with enhanced sync
        ...
        # CRITICAL FIX: Enhanced collision settings with better error handling and verification
    ...
    def _get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "phase_index": self.get_current_phase_index(),  # CRITICAL FIX: Include proper phase index
    ...
    def _force_curriculum_level_sync(self):
        ...
        # Enhanced synchronization with multiple attempts and fallbacks
        sync_success = False
        attempts = 0
        max_attempts = 5
        ...
        # FIXED: Update last_curriculum_level to prevent smoothing issues in future calls
    ...
    def verify_curriculum_level(self) -> bool:
        ...
        # CRITICAL FIX: Add cooldown to prevent continuous fighting
